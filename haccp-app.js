...
function recordReview(){
 const p={...defaultPlan,...(db.managementPlan||{})};p.reviews=Array.isArray(p.reviews)?p.reviews:[];const latest=p.reviews[0];
 const hs=[...db.harvests].sort((a,b)=>String(b.date||'').localeCompare(String(a.date||''))),bs=[...db.bottles].sort((a,b)=>String(b.date||'').localeCompare(String(a.date||''))),ss=[...db.sales].sort((a,b)=>String(b.date||'').localeCompare(String(a.date||'')));
 ...
 shell('記録の確認・見直し',`<div class="panel record-review-summary">${latest?`<b>最終確認：${esc(fmtDate(latest.date))}</b><div class="hint">${latest.changed?'改善・変更の必要あり':'問題なし'} ／ 確認者：${esc(latest.reviewer||'-')}</div>`:'<b>確認履歴はまだありません</b>'}<div class="hint">この1ページで実際の記録を確認してから、一番下で今回の確認結果を保存できます。</div></div>
