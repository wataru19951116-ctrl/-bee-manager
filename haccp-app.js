...
const defaultPlan={responsibleName:'',reviewDate:'',facility:'',notes:''};
...
function normalizeDB(x){return {version:3,harvests:Array.isArray(x.harvests)?x.harvests:[],bottles:Array.isArray(x.bottles)?x.bottles:[],sales:Array.isArray(x.sales)?x.sales:[],processedBeeRecordIds:Array.isArray(x.processedBeeRecordIds)?x.processedBeeRecordIds.map(String):[],managementPlan:{...defaultPlan,...(x.managementPlan||{})}}}
...
function managementPlan(){const p={...defaultPlan,...(db.managementPlan||{})};shell('衛生管理計画',`<div class="panel"><b>小さな蜜の舎　一般衛生管理計画</b><div class="hint">厚生労働省掲載「はちみつの瓶詰め等の製造におけるHACCP導入の手引書」の一般衛生管理項目を参考にしたアプリ用計画です。</div></div>${planItems.map((x,i)=>`<div class="item"><b>${i+1}. ${esc(x[0])}</b><div class="pending-meta">管理方法：${esc(x[1])}<br>頻度：${esc(x[2])}</div></div>`).join('')}<div class="field"><label>作業場・施設名</label><input id="planFacility" value="${esc(p.facility)}" placeholder="例：小さな蜜の舎 瓶詰め作業場"></div><div class="field"><label>衛生管理責任者</label><input id="planResponsible" value="${esc(p.responsibleName)}" placeholder="氏名"></div><div class="field"><label>計画の確認・見直し日</label><input id="planReviewDate" type="date" value="${esc(p.reviewDate)}"></div><div class="field"><label>計画上の補足・ルール</label><textarea id="planNotes" rows="4">${esc(p.notes)}</textarea></div><button class="action primary" onclick="saveManagementPlan()">衛生管理計画を保存</button><button class="action secondary" onclick="page('records')">記録ページへ戻る</button>`)}
function saveManagementPlan(){db.managementPlan={facility:q('planFacility').value.trim(),responsibleName:q('planResponsible').value.trim(),reviewDate:q('planReviewDate').value,notes:q('planNotes').value.trim()};save();alert('衛生管理計画を保存しました。');records()}
