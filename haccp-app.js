...
function recordReview(){const p={...defaultPlan,...(db.managementPlan||{})};p.reviews=Array.isArray(p.reviews)?p.reviews:[];const latest=p.reviews[0];shell('記録の確認・見直し',`<div class="panel record-review-summary">${latest?`<b>最終確認：${esc(fmtDate(latest.date))}</b><div class="hint">${latest.changed?'改善・変更の必要あり':'問題なし'} ／ 確認者：${esc(latest.reviewer||'-')}</div>`:'<b>確認履歴はまだありません</b>'}<div class="hint">採蜜・瓶詰め・衛生チェック・異常時対応などの記録を振り返り、問題や傾向がないか確認します。</div></div><div class="field"><label>確認日</label><input id="reviewDate" type="date" value="${today()}"></div><div class="field"><label>確認結果</label><select id="reviewChanged"><option value="no">問題なし</option><option value="yes">改善・変更の必要あり</option></select></div><div class="field"><label>確認内容・改善内容</label><textarea id="reviewContent" rows="4" placeholder="確認した内容、気になった点、改善する内容など"></textarea></div><div class="field"><label>確認者</label><input id="reviewReviewer" value="${esc(p.responsibleName)}" placeholder="氏名"></div><button class="action primary" onclick="addRecordReview()">確認履歴を保存</button><h2 class="section">確認・見直し履歴</h2>${p.reviews.length?p.reviews.map(r=>`<div class="item"><b>${esc(fmtDate(r.date))}　${r.changed?'改善・変更の必要あり':'問題なし'}</b><div class="pending-meta">確認者：${esc(r.reviewer||'-')}${r.content?`<br>内容：${esc(r.content)}`:''}</div></div>`).join(''):'<div class="empty panel">確認履歴はありません。</div>'}<button class="action secondary" onclick="page('records')">記録ページへ戻る</button>`)}
