...
function records(){
  ...
  shell('記録',`<div class="panel plan-summary"><b>📋 衛生管理計画</b><div class="hint">はちみつの瓶詰め等のHACCP手引書を参考に、一般衛生管理の計画を確認・保存します。</div><button class="record-toggle" style="margin-top:10px" onclick="managementPlan()">衛生管理計画を見る・編集する</button></div><div class="report-filter"><div class="row"><div class="field"><label>開始日</label><input id="rFrom" type="date" value="${esc(minDate)}"></div><div class="field"><label>終了日</label><input id="rTo" type="date" value="${esc(maxDate)}"></div></div><div class="toolbar"><button onclick="exportCSV()">CSVを書き出す</button><button onclick="printReport()">PDF提出用</button></div><div class="hint">「PDF提出用」は衛生管理計画・衛生チェック・異常時対応・確認者・ロット追跡を含む印刷画面を開きます。</div></div>
...
function managementPlan(){const p={...defaultPlan,...(db.managementPlan||{})};p.reviews=Array.isArray(p.reviews)?p.reviews:[];const latest=p.reviews[0];shell('衛生管理計画',`<div class="panel plan-identity"><b>HACCP管理　一般衛生管理計画</b><div class="plan-person"><span>責任者名</span><strong>${esc(p.responsibleName||'未登録')}</strong></div><div class="plan-person"><span>作業場所名</span><strong>${esc(p.facility||'未登録')}</strong></div><div class="hint">厚生労働省掲載「はちみつの瓶詰め等の製造におけるHACCP導入の手引書」の一般衛生管理項目を参考にしたアプリ用計画です。</div></div>${planItems.map((x,i)=>`<div class="item"><b>${i+1}. ${esc(x[0])}</b><div class="pending-meta">管理方法：${esc(x[1])}<br>頻度：${esc(x[2])}</div></div>`).join('')}<h2 class="section">基本情報</h2><div class="field"><label>作業場所名</label><input id="planFacility" value="${esc(p.facility)}" placeholder="例：瓶詰め作業場"></div><div class="field"><label>責任者名</label><input id="planResponsible" value="${esc(p.responsibleName)}" placeholder="氏名"></div><div class="field"><label>計画上の補足・ルール</label><textarea id="planNotes" rows="4">${esc(p.notes)}</textarea></div><button class="action primary" onclick="saveManagementPlan()">衛生管理計画を保存</button><h2 class="section">計画の確認・見直し</h2><div class="panel">${latest?`<b>最終見直し：${esc(fmtDate(latest.date))}</b><div class="hint">${latest.changed?'変更あり':'変更なし'} ／ 確認者：${esc(latest.reviewer||'-')}</div>`:'<b>見直し履歴はまだありません</b>'}<div class="hint">実際に計画を確認・見直したときだけ記録します。過去の履歴は残ります。</div></div><div class="field"><label>見直し日</label><input id="reviewDate" type="date" value="${today()}"></div><div class="field"><label>結果</label><select id="reviewChanged"><option value="no">変更なし</option><option value="yes">変更あり</option></select></div><div class="field"><label>見直し内容</label><textarea id="reviewContent" rows="3" placeholder="変更なしの場合は、確認した内容などを必要に応じて入力"></textarea></div><div class="field"><label>確認者</label><input id="reviewReviewer" value="${esc(p.responsibleName)}" placeholder="氏名"></div><button class="action secondary" onclick="addPlanReview()">見直し履歴を追加</button><h2 class="section">見直し履歴</h2>${p.reviews.length?p.reviews.map(r=>`<div class="item"><b>${esc(fmtDate(r.date))}　${r.changed?'変更あり':'変更なし'}</b><div class="pending-meta">確認者：${esc(r.reviewer||'-')}${r.content?`<br>内容：${esc(r.content)}`:''}</div></div>`).join(''):'<div class="empty panel">見直し履歴はありません。</div>'}<button class="action secondary" onclick="page('records')">記録ページへ戻る</button>`)}
...
function addPlanReview(){const date=q('reviewDate').value,changed=q('reviewChanged').value==='yes',content=q('reviewContent').value.trim(),reviewer=q('reviewReviewer').value.trim();if(!date)return alert('見直し日を入れてください。');if(!reviewer)return alert('確認者を入れてください。');if(changed&&!content)return alert('変更ありの場合は、見直し内容を入れてください。');const p={...defaultPlan,...(db.managementPlan||{})};p.reviews=Array.isArray(p.reviews)?p.reviews:[];p.reviews.unshift({id:uid('pr'),date,changed,content,reviewer,createdAt:Date.now()});p.reviewDate=date;db.managementPlan=p;save();alert('見直し履歴を保存しました。');managementPlan()}
