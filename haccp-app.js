...
const defaultPlan={responsibleName:'',reviewDate:'',facility:'',notes:'',reviews:[]};
const defaultDB={version:3,harvests:[],bottles:[],sales:[],processedBeeRecordIds:[],managementPlan:{...defaultPlan}};
let db=loadDB();
...
function normalizeDB(x){const mp={...defaultPlan,...(x.managementPlan||{})};mp.reviews=Array.isArray(mp.reviews)?mp.reviews:[];return {version:4,harvests:Array.isArray(x.harvests)?x.harvests:[],bottles:Array.isArray(x.bottles)?x.bottles:[],sales:Array.isArray(x.sales)?x.sales:[],processedBeeRecordIds:Array.isArray(x.processedBeeRecordIds)?x.processedBeeRecordIds.map(String):[],managementPlan:mp}}
...
function startHarvestForm(){
  ...
  <h2 class="section">確認・異常時対応</h2>
  <div class="field"><label>異常の有無</label><select id="hAbnormal"><option value="no">異常なし</option><option value="yes">異常あり</option></select></div>
  <div class="field"><label>異常・特記事項</label><textarea id="hMemo" rows="3" placeholder="例：器具を床に落とした、異臭を確認した など"></textarea></div>
  <div class="field"><label>異常時の対応内容</label><textarea id="hAction" rows="3" placeholder="例：器具を再洗浄・乾燥してから作業を再開"></textarea></div>
  <div class="row"><div class="field"><label>作業者</label><input id="hOperator" placeholder="氏名"></div><div class="field"><label>確認者</label><input id="hReviewer" placeholder="責任者名"></div></div>
  <button class="action primary" onclick="saveHarvest()">採蜜記録を保存</button>`)
}
function saveHarvest(){
  ...
  const abnormal=q('hAbnormal').value==='yes',correctiveAction=q('hAction').value.trim();if(abnormal&&!correctiveAction)return alert('「異常あり」の場合は、異常時の対応内容を入力してください。');const rec={id:uid('h'),date,lot,kg:q('hKg').value,container:q('hContainer').value.trim(),mixed,sourceBeeRecordIds:pendingSelection.map(x=>x.id),hives,steps,checks,abnormal,memo:q('hMemo').value.trim(),correctiveAction,operator:q('hOperator').value.trim(),reviewer:q('hReviewer').value.trim(),createdAt:Date.now()};
...
function startBottleForm(){
  ...
  <h2 class="section">確認・異常時対応</h2>
  <div class="field"><label>異常の有無</label><select id="bAbnormal"><option value="no">異常なし</option><option value="yes">異常あり</option></select></div>
  <div class="field"><label>異常・特記事項</label><textarea id="bMemo" rows="3" placeholder="例：瓶の破損、異物混入のおそれ など"></textarea></div>
  <div class="field"><label>異常時の対応内容</label><textarea id="bAction" rows="3" placeholder="例：該当瓶を除外し、器具と作業台を再確認"></textarea></div>
  <div class="row"><div class="field"><label>作業者</label><input id="bOperator" placeholder="氏名"></div><div class="field"><label>確認者</label><input id="bReviewer" placeholder="責任者名"></div></div>
  <button class="action primary" onclick="saveBottle()">瓶詰め記録を保存</button>`);updateBottleTotals()
}
...
function saveBottle(){
  ...
  const abnormal=q('bAbnormal').value==='yes',correctiveAction=q('bAction').value.trim();if(abnormal&&!correctiveAction)return alert('「異常あり」の場合は、異常時の対応内容を入力してください。');const rec={id:uid('b'),date,lot,srcIds,srcLots,grams,count,allocations,steps,checks,abnormal,memo:q('bMemo').value.trim(),correctiveAction,operator:q('bOperator').value.trim(),reviewer:q('bReviewer').value.trim(),createdAt:Date.now()};db.bottles.unshift(rec);for(const a of allocations){if(a.finish){const h=db.harvests.find(x=>String(x.id)===a.harvestId);if(h){const after=Math.max(0,a.beforeGrams-a.productGrams-a.lossGrams);a.finalLossGrams=after;h.closed=true;h.closedAt=Date.now()}}}save();bottleSelection=[];
...
function records(){
  ...
  const p={...defaultPlan,...(db.managementPlan||{})};p.reviews=Array.isArray(p.reviews)?p.reviews:[];const latest=p.reviews[0];
  shell('記録',`<div class="panel plan-summary"><b>📋 衛生管理計画</b><div class="hint">一般衛生管理の計画、責任者名、作業場所名を確認・編集できます。</div><button class="record-toggle" style="margin-top:10px" onclick="managementPlan()">衛生管理計画を見る・編集する</button></div>
  <div class="panel record-review-summary"><b>🔎 記録の確認・見直し</b>${latest?`<div class="review-latest"><strong>最終確認：${esc(fmtDate(latest.date))}</strong><span>${latest.changed?'改善・変更の必要あり':'問題なし'} ／ 確認者：${esc(latest.reviewer||'-')}</span></div>`:'<div class="hint">確認履歴はまだありません。</div>'}<div class="hint">採蜜・瓶詰め・衛生チェックなどの記録を振り返り、問題や傾向がないか確認します。必要があれば衛生管理計画の見直しにつなげます。</div><button class="record-toggle" style="margin-top:10px" onclick="recordReview()">記録を確認・見直す</button></div>
  <div class="report-filter"><div class="row"><div class="field"><label>開始日</label><input id="rFrom" type="date" value="${esc(minDate)}"></div><div class="field"><label>終了日</label><input id="rTo" type="date" value="${esc(maxDate)}"></div></div><div class="toolbar"><button onclick="exportCSV()">CSVを書き出す</button><button onclick="printReport()">PDF提出用</button></div><div class="hint">「PDF提出用」は衛生管理計画・記録の確認履歴・衛生チェック・異常時対応・確認者・ロット追跡を含む印刷画面を開きます。</div></div>
...
function harvestRecordHtml(h,active=true){const rem=harvestRemainingGrams(h),used=harvestUsage(h.id);return `<div class="item"><div class="record-head"><div><b>🍯 ${esc(h.lot)}</b><div class="mini">${esc(fmtDate(h.date))} ／ ${h.mixed?'複数群混合':'非混合'}</div></div><span class="remaining ${rem>0?'okbg':'mutedbg'}">残り ${esc(rem)}g</span></div><div class="pillline">${(h.hives||[]).map(x=>`<span class="badge blue">${esc(x.name)}</span>`).join('')}</div><div class="pending-meta">採蜜時：${esc(harvestInitialGrams(h))}g${h.container?` ／ 容器：${esc(h.container)}`:''}<br>瓶詰め使用：${esc(used.product)}g ／ ロス：${esc(used.loss)}g<br>衛生チェック ${(h.checks||[]).filter(Boolean).length}/${(h.checks||[]).length||harvestChecks.length}${h.abnormal?'<br><b>異常あり</b>':'<br>異常なし'}${h.memo?`<br>特記事項：${esc(h.memo)}`:''}${h.correctiveAction?`<br>対応：${esc(h.correctiveAction)}`:''}${h.operator?`<br>作業者：${esc(h.operator)}`:''}${h.reviewer?` ／ 確認者：${esc(h.reviewer)}`:''}</div><button class="delete-btn" onclick="deleteHarvest('${esc(h.id)}')">この採蜜記録を削除</button></div>`}
function bottleRecordHtml(b,active=true){const rem=bottleRemainingCount(b),sold=bottleSoldCount(b);return `<div class="item"><div class="record-head"><div><b>🫙 ${esc(b.lot)}</b><div class="mini">${esc(fmtDate(b.date))}</div></div><span class="remaining ${rem>0?'okbg':'mutedbg'}">残り ${esc(rem)}本${b.grams?`・${esc(bottleRemainingGrams(b))}g`:''}</span></div><div class="pillline">${(b.srcLots||[]).map(x=>`<span class="badge gold">${esc(x)}</span>`).join('')}</div><div class="pending-meta">製造：${esc(b.grams)}g × ${esc(b.count)}本 ／ 販売済み：${esc(sold)}本${Array.isArray(b.allocations)?`<br>使用：${b.allocations.map(a=>`${esc(a.lot)} ${esc(a.productGrams)}g`).join(' ／ ')}<br>ロス：${b.allocations.reduce((n,a)=>n+Number(a.lossGrams||0)+Number(a.finalLossGrams||0),0)}g`:''}<br>衛生チェック ${(b.checks||[]).filter(Boolean).length}/${(b.checks||[]).length||bottleChecks.length}${b.abnormal?'<br><b>異常あり</b>':'<br>異常なし'}${b.memo?`<br>特記事項：${esc(b.memo)}`:''}${b.correctiveAction?`<br>対応：${esc(b.correctiveAction)}`:''}${b.operator?`<br>作業者：${esc(b.operator)}`:''}${b.reviewer?` ／ 確認者：${esc(b.reviewer)}`:''}</div><button class="delete-btn" onclick="deleteBottle('${esc(b.id)}')">この瓶詰め記録を削除</button></div>`}
...
const planItems=[
  ['原材料の確認','採蜜ロット・外観・異臭・異物などを確認','使用・受入の都度'],
  ['原材料・保管場所の衛生管理','蜂蜜容器と保管場所を清潔に保ち、汚染・害虫・水濡れ等を確認','最低週1回／使用時'],
  ['作業場の衛生管理','作業台・床・周辺を清潔にし、食品へ汚染が及ばない状態を確認','作業日ごと'],
  ['製造時の衛生管理','器具・瓶・蓋の清潔、異物混入防止、作業前後の状態を確認','採蜜・瓶詰め作業の都度'],
  ['製品保管場所の衛生管理','瓶詰め後の製品を清潔な場所で保管し、破損・汚染等を確認','保管中'],
  ['作業者の衛生管理','体調、清潔な服装、必要に応じて作業を中止する判断を行う','作業日ごと'],
  ['手洗いの実施','作業開始前、汚染のおそれがある行為の後に手洗いを行う','作業の都度']
];
function managementPlan(){const p={...defaultPlan,...(db.managementPlan||{})};shell('衛生管理計画',`<div class="panel plan-identity"><b>HACCP管理　一般衛生管理計画</b><div class="plan-person"><span>責任者名</span><strong>${esc(p.responsibleName||'未登録')}</strong></div><div class="plan-person"><span>作業場所名</span><strong>${esc(p.facility||'未登録')}</strong></div><div class="hint">厚生労働省掲載「はちみつの瓶詰め等の製造におけるHACCP導入の手引書」の一般衛生管理項目を参考にしたアプリ用計画です。</div></div>${planItems.map((x,i)=>`<div class="item"><b>${i+1}. ${esc(x[0])}</b><div class="pending-meta">管理方法：${esc(x[1])}<br>頻度：${esc(x[2])}</div></div>`).join('')}<h2 class="section">基本情報</h2><div class="field"><label>作業場所名</label><input id="planFacility" value="${esc(p.facility)}" placeholder="例：瓶詰め作業場"></div><div class="field"><label>責任者名</label><input id="planResponsible" value="${esc(p.responsibleName)}" placeholder="氏名"></div><div class="field"><label>計画上の補足・ルール</label><textarea id="planNotes" rows="4">${esc(p.notes)}</textarea></div><button class="action primary" onclick="saveManagementPlan()">衛生管理計画を保存</button><div class="panel" style="margin-top:14px"><b>記録の確認・見直し</b><div class="hint">採蜜・瓶詰めなどの記録の確認は「記録」画面から行います。</div><button class="record-toggle" style="margin-top:8px" onclick="recordReview()">記録の確認・見直しへ</button></div><button class="action secondary" onclick="page('records')">記録ページへ戻る</button>`)}
function saveManagementPlan(){const old={...defaultPlan,...(db.managementPlan||{})};db.managementPlan={...old,facility:q('planFacility').value.trim(),responsibleName:q('planResponsible').value.trim(),notes:q('planNotes').value.trim(),reviews:Array.isArray(old.reviews)?old.reviews:[]};save();alert('衛生管理計画を保存しました。');managementPlan()}
function reviewHarvestCard(h){const done=(h.checks||[]).filter(Boolean).length,total=(h.checks||[]).length||harvestChecks.length;return `<div class="review-card"><div class="review-card-head"><b>🍯 ${esc(h.lot)}</b><span>${esc(fmtDate(h.date))}</span></div><div class="pending-meta">採蜜元：${esc((h.hives||[]).map(x=>x.name).join('・')||'-')} ／ 採蜜量：${esc(h.kg||'-')}kg<br>衛生チェック：${done}/${total} ／ ${h.abnormal?'<strong class="danger">異常あり</strong>':'異常なし'}${h.correctiveAction?`<br>是正・対応：${esc(h.correctiveAction)}`:''}${h.memo?`<br>特記事項：${esc(h.memo)}`:''}<br>作業者：${esc(h.operator||'-')} ／ 確認者：${esc(h.reviewer||'-')}</div></div>`}
function reviewBottleCard(b){const done=(b.checks||[]).filter(Boolean).length,total=(b.checks||[]).length||bottleChecks.length;return `<div class="review-card"><div class="review-card-head"><b>🫙 ${esc(b.lot)}</b><span>${esc(fmtDate(b.date))}</span></div><div class="pending-meta">採蜜ロット：${esc((b.srcLots||[]).join('＋')||'-')} ／ ${esc(b.grams)}g × ${esc(b.count)}本<br>衛生チェック：${done}/${total} ／ ${b.abnormal?'<strong class="danger">異常あり</strong>':'異常なし'}${b.correctiveAction?`<br>是正・対応：${esc(b.correctiveAction)}`:''}${b.memo?`<br>特記事項：${esc(b.memo)}`:''}<br>作業者：${esc(b.operator||'-')} ／ 確認者：${esc(b.reviewer||'-')}</div></div>`}
function reviewSaleCard(s){return `<div class="review-card"><div class="review-card-head"><b>🧾 ${esc(s.lot||'製品ロット未設定')}</b><span>${esc(fmtDate(s.date))}</span></div><div class="pending-meta">販売数：${esc(s.count||0)}本 ／ 販売先・注文番号：${esc(s.dest||'-')}</div></div>`}
let reviewSearch={mode:'all',from:'',to:'',lot:''};
function dateInRange(date,from,to){const d=String(date||'');return (!from||d>=from)&&(!to||d<=to)}
function reviewRelatedByLot(term){
 const t=String(term||'').trim().toUpperCase();
 if(!t)return {harvests:[...db.harvests],bottles:[...db.bottles],sales:[...db.sales],matched:''};
 const harvestIds=new Set(),harvestLots=new Set(),bottleLots=new Set();
 db.harvests.forEach(h=>{if(String(h.lot||'').toUpperCase().includes(t)){harvestIds.add(String(h.id));harvestLots.add(String(h.lot||''))}});
 db.bottles.forEach(b=>{const direct=String(b.lot||'').toUpperCase().includes(t);const srcMatch=(b.srcLots||[]).some(x=>String(x).toUpperCase().includes(t));if(direct||srcMatch){bottleLots.add(String(b.lot||''));(b.srcIds||[]).forEach(x=>harvestIds.add(String(x)));(b.srcLots||[]).forEach(x=>harvestLots.add(String(x)));(b.allocations||[]).forEach(a=>{harvestIds.add(String(a.harvestId||''));harvestLots.add(String(a.lot||''))})}});
 db.sales.forEach(s=>{if(String(s.lot||'').toUpperCase().includes(t))bottleLots.add(String(s.lot||''))});
 // 関連ロットを相互にたどる（製品→採蜜、採蜜→製品→販売）
 let changed=true;while(changed){changed=false;db.bottles.forEach(b=>{const uses=[...(b.srcIds||[]).map(String),...(b.allocations||[]).map(a=>String(a.harvestId||''))].some(x=>harvestIds.has(x))||(b.srcLots||[]).some(x=>harvestLots.has(String(x)));if(uses||bottleLots.has(String(b.lot||''))){if(!bottleLots.has(String(b.lot||''))){bottleLots.add(String(b.lot||''));changed=true}(b.srcIds||[]).forEach(x=>{if(!harvestIds.has(String(x))){harvestIds.add(String(x));changed=true}});(b.srcLots||[]).forEach(x=>{if(!harvestLots.has(String(x))){harvestLots.add(String(x));changed=true}});(b.allocations||[]).forEach(a=>{if(!harvestIds.has(String(a.harvestId||''))){harvestIds.add(String(a.harvestId||''));changed=true}if(a.lot&&!harvestLots.has(String(a.lot))){harvestLots.add(String(a.lot));changed=true}})}})}
 const hs=db.harvests.filter(h=>harvestIds.has(String(h.id))||harvestLots.has(String(h.lot||'')));
 const bs=db.bottles.filter(b=>bottleLots.has(String(b.lot||'')));
 const ss=db.sales.filter(s=>bottleLots.has(String(s.lot||'')));
 return {harvests:hs,bottles:bs,sales:ss,matched:t};
}
function applyReviewDateSearch(){const from=q('reviewFrom').value,to=q('reviewTo').value;if(from&&to&&from>to)return alert('開始日は終了日より前の日付にしてください。');reviewSearch={mode:'date',from,to,lot:''};recordReview()}
function applyReviewLotSearch(){const lot=q('reviewLot').value.trim();if(!lot)return alert('ロット番号を入力してください。');reviewSearch={mode:'lot',from:'',to:'',lot};recordReview()}
function clearReviewSearch(){reviewSearch={mode:'all',from:'',to:'',lot:''};recordReview()}
function recordReview(){
 const p={...defaultPlan,...(db.managementPlan||{})};p.reviews=Array.isArray(p.reviews)?p.reviews:[];const latest=p.reviews[0];
 let hs,bs,ss;
 if(reviewSearch.mode==='lot'){const r=reviewRelatedByLot(reviewSearch.lot);hs=r.harvests;bs=r.bottles;ss=r.sales}else if(reviewSearch.mode==='date'){hs=db.harvests.filter(x=>dateInRange(x.date,reviewSearch.from,reviewSearch.to));bs=db.bottles.filter(x=>dateInRange(x.date,reviewSearch.from,reviewSearch.to));ss=db.sales.filter(x=>dateInRange(x.date,reviewSearch.from,reviewSearch.to))}else{hs=[...db.harvests];bs=[...db.bottles];ss=[...db.sales]}
 hs=[...hs].sort((a,b)=>String(b.date||'').localeCompare(String(a.date||'')));bs=[...bs].sort((a,b)=>String(b.date||'').localeCompare(String(a.date||'')));ss=[...ss].sort((a,b)=>String(b.date||'').localeCompare(String(a.date||'')));
 const abnormalities=[...hs.filter(x=>x.abnormal).map(x=>({type:'採蜜',lot:x.lot,date:x.date,action:x.correctiveAction,memo:x.memo})),...bs.filter(x=>x.abnormal).map(x=>({type:'瓶詰め',lot:x.lot,date:x.date,action:x.correctiveAction,memo:x.memo}))].sort((a,b)=>String(b.date||'').localeCompare(String(a.date||'')));
 const scope=reviewSearch.mode==='lot'?`ロット「${esc(reviewSearch.lot)}」に関係する記録`:reviewSearch.mode==='date'?`${esc(reviewSearch.from||'最初')} ～ ${esc(reviewSearch.to||'最新')} の記録`:'すべての記録';
 shell('記録の確認・見直し',`<div class="panel record-review-summary">${latest?`<b>最終確認：${esc(fmtDate(latest.date))}</b><div class="hint">${latest.changed?'改善・変更の必要あり':'問題なし'} ／ 確認者：${esc(latest.reviewer||'-')}</div>`:'<b>確認履歴はまだありません</b>'}<div class="hint">検索した記録をこの1ページで確認し、一番下で今回の確認結果を保存できます。</div></div>
 <div class="panel review-search"><b>🔍 確認する記録を検索</b><div class="hint">日付の範囲、またはロット番号から絞り込めます。</div><div class="row" style="margin-top:10px"><div class="field"><label>開始日</label><input id="reviewFrom" type="date" value="${esc(reviewSearch.from)}"></div><div class="field"><label>終了日</label><input id="reviewTo" type="date" value="${esc(reviewSearch.to)}"></div></div><button class="action secondary" onclick="applyReviewDateSearch()">日付範囲で検索</button><div class="field" style="margin-top:12px"><label>ロット番号</label><input id="reviewLot" value="${esc(reviewSearch.lot)}" placeholder="例：H-260908-01 / P-260908-01"></div><button class="action secondary" onclick="applyReviewLotSearch()">ロット番号から関連記録を検索</button><button class="record-toggle" style="margin-top:10px" onclick="clearReviewSearch()">検索を解除して全件表示</button></div>
 <div class="panel review-scope"><b>現在の確認範囲</b><div class="pending-meta">${scope}</div></div>
 <div class="review-index"><span>採蜜 ${hs.length}件</span><span>瓶詰め ${bs.length}件</span><span>販売 ${ss.length}件</span><span class="${abnormalities.length?'review-alert':''}">異常 ${abnormalities.length}件</span></div>
 <h2 class="section">🍯 採蜜記録</h2>${hs.length?hs.map(reviewHarvestCard).join(''):'<div class="empty panel">該当する採蜜記録はありません。</div>'}
 <h2 class="section">🫙 瓶詰め記録</h2>${bs.length?bs.map(reviewBottleCard).join(''):'<div class="empty panel">該当する瓶詰め記録はありません。</div>'}
 <h2 class="section">🧾 在庫・販売記録</h2>${bs.length?bs.map(b=>`<div class="review-card"><div class="review-card-head"><b>${esc(b.lot)}</b><span>残り ${esc(bottleRemainingCount(b))}本</span></div><div class="pending-meta">製造 ${esc(b.count||0)}本 ／ 販売済み ${esc(bottleSoldCount(b))}本</div></div>`).join(''):'<div class="empty panel">該当する製品ロットはありません。</div>'}${ss.length?`<h3 class="subsection">販売履歴</h3>${ss.map(reviewSaleCard).join('')}`:''}
 <h2 class="section">⚠️ 異常・是正記録</h2>${abnormalities.length?abnormalities.map(x=>`<div class="review-card review-abnormal"><div class="review-card-head"><b>${esc(x.type)}・${esc(x.lot)}</b><span>${esc(fmtDate(x.date))}</span></div><div class="pending-meta">${x.memo?`内容：${esc(x.memo)}<br>`:''}是正・対応：${esc(x.action||'記載なし')}</div></div>`).join(''):'<div class="panel review-ok">✓ この確認範囲に「異常あり」の記録はありません。</div>'}
 <h2 class="section review-form-title">今回の確認結果</h2><div class="panel review-form"><div class="field"><label>確認した範囲</label><input id="reviewScope" value="${scope}" readonly></div><div class="field"><label>確認日</label><input id="reviewDate" type="date" value="${today()}"></div><div class="field"><label>確認結果</label><select id="reviewChanged"><option value="no">問題なし</option><option value="yes">改善・変更の必要あり</option></select></div><div class="field"><label>確認内容・改善内容</label><textarea id="reviewContent" rows="4" placeholder="記録を確認して気になった点、改善する内容など"></textarea></div><div class="field"><label>確認者</label><input id="reviewReviewer" value="${esc(p.responsibleName)}" placeholder="氏名"></div><button class="action primary" onclick="addRecordReview()">確認履歴を保存</button></div>
 <h2 class="section">確認・見直し履歴</h2>${p.reviews.length?p.reviews.map(r=>`<div class="item"><b>${esc(fmtDate(r.date))}　${r.changed?'改善・変更の必要あり':'問題なし'}</b><div class="pending-meta">確認者：${esc(r.reviewer||'-')}${r.scope?`<br>確認範囲：${esc(r.scope)}`:''}${r.content?`<br>内容：${esc(r.content)}`:''}</div></div>`).join(''):'<div class="empty panel">確認履歴はありません。</div>'}<button class="action secondary" onclick="page('records')">記録ページへ戻る</button>`)
}
function addRecordReview(){const date=q('reviewDate').value,changed=q('reviewChanged').value==='yes',content=q('reviewContent').value.trim(),reviewer=q('reviewReviewer').value.trim(),scope=q('reviewScope')?.value||'すべての記録';if(!date)return alert('確認日を入れてください。');if(!reviewer)return alert('確認者を入れてください。');if(changed&&!content)return alert('「改善・変更の必要あり」の場合は、内容を入れてください。');const p={...defaultPlan,...(db.managementPlan||{})};p.reviews=Array.isArray(p.reviews)?p.reviews:[];p.reviews.unshift({id:uid('rr'),date,changed,content,reviewer,scope,createdAt:Date.now()});p.reviewDate=date;db.managementPlan=p;save();alert('記録の確認履歴を保存しました。');recordReview()}
...
function checkListHtml(labels,values){return labels.map((x,i)=>`${i+1}. ${esc(x)}：${values?.[i]?'良':'否'}`).join('<br>')}
function printReport(){
  const r=reportRange(),w=window.open('','_blank');if(!w)return alert('印刷画面を開けませんでした。Safariのポップアップ設定を確認してください。');
  const p={...defaultPlan,...(db.managementPlan||{})};
  const rowsH=r.harvests.map(h=>`<tr><td>${esc(h.date)}</td><td>${esc(h.lot)}</td><td>${esc((h.hives||[]).map(x=>x.name).join('・'))}</td><td>${esc(h.kg||'-')}kg<br>残 ${esc(harvestRemainingGrams(h))}g</td><td>${checkListHtml(harvestChecks,h.checks)}</td><td>${h.abnormal?'異常あり':'異常なし'}${h.memo?`<br>${esc(h.memo)}`:''}${h.correctiveAction?`<br><b>対応：</b>${esc(h.correctiveAction)}`:''}</td><td>${esc(h.operator||'-')}<br>確認：${esc(h.reviewer||'-')}</td></tr>`).join('')||'<tr><td colspan="7">記録なし</td></tr>';
  const rowsB=r.bottles.map(b=>`<tr><td>${esc(b.date)}</td><td>${esc(b.lot)}</td><td>${esc((b.srcLots||[]).join('＋'))}<br>${Array.isArray(b.allocations)?b.allocations.map(a=>`${esc(a.lot)} ${esc(a.productGrams)}g`).join('<br>'):''}</td><td>${esc(b.grams)}g×${esc(b.count)}本<br>残 ${esc(bottleRemainingCount(b))}本</td><td>${checkListHtml(bottleChecks,b.checks)}</td><td>${b.abnormal?'異常あり':'異常なし'}${b.memo?`<br>${esc(b.memo)}`:''}${b.correctiveAction?`<br><b>対応：</b>${esc(b.correctiveAction)}`:''}</td><td>${esc(b.operator||'-')}<br>確認：${esc(b.reviewer||'-')}</td></tr>`).join('')||'<tr><td colspan="7">記録なし</td></tr>';
  ...
  const planRows=planItems.map((x,i)=>`<tr><td>${i+1}. ${esc(x[0])}</td><td>${esc(x[1])}</td><td>${esc(x[2])}</td></tr>`).join('');
  const reviewRows=(Array.isArray(p.reviews)?p.reviews:[]).map(x=>`<tr><td>${esc(x.date)}</td><td>${x.changed?'改善・変更の必要あり':'問題なし'}</td><td>${esc(x.content||'-')}</td><td>${esc(x.reviewer||'-')}</td></tr>`).join('')||'<tr><td colspan="4">確認履歴なし</td></tr>';
  w.document.write(`<!doctype html><html lang="ja"><head><meta charset="utf-8"><title>小さな蜜の舎 HACCP衛生管理記録</title><style>body{font-family:-apple-system,'Hiragino Sans','Yu Gothic',sans-serif;color:#222;padding:22px}h1{font-size:21px;margin-bottom:4px}h2{font-size:15px;margin-top:24px;border-bottom:2px solid #444;padding-bottom:4px}p{font-size:11px;line-height:1.5}table{width:100%;border-collapse:collapse;font-size:9px;margin-top:8px}th,td{border:1px solid #888;padding:5px;vertical-align:top;line-height:1.45}th{background:#eee}.meta{display:grid;grid-template-columns:1fr 1fr;gap:4px;font-size:11px}.note{font-size:9px;color:#555}@media print{body{padding:0}@page{size:A4 landscape;margin:10mm}h2{break-after:avoid}table{break-inside:auto}tr{break-inside:avoid}}</style></head><body><h1>小さな蜜の舎 HACCP衛生管理記録</h1><div class="meta"><div>対象期間：${esc(r.from)} ～ ${esc(r.to)}</div><div>作業場：${esc(p.facility||'-')}</div><div>責任者名：${esc(p.responsibleName||'-')}</div><div>最新記録確認日：${esc((p.reviews&&p.reviews[0]?.date)||p.reviewDate||'-')}</div></div><h2>1. 衛生管理計画</h2><table><thead><tr><th>管理項目</th><th>管理方法</th><th>頻度</th></tr></thead><tbody>${planRows}</tbody></table>${p.notes?`<p><b>補足：</b>${esc(p.notes)}</p>`:''}<h2>2. 記録の確認・見直し履歴</h2><table><thead><tr><th>確認日</th><th>確認結果</th><th>確認・改善内容</th><th>確認者</th></tr></thead><tbody>${reviewRows}</tbody></table><h2>3. 採蜜・衛生管理記録</h2><table><thead><tr><th>日付</th><th>採蜜ロット</th><th>由来群</th><th>数量・残量</th><th>衛生チェック</th><th>異常・対応</th><th>作業者・確認者</th></tr></thead><tbody>${rowsH}</tbody></table><h2>4. 瓶詰め・衛生管理記録</h2><table><thead><tr><th>日付</th><th>製品ロット</th><th>使用採蜜ロット</th><th>製造・在庫</th><th>衛生チェック</th><th>異常・対応</th><th>作業者・確認者</th></tr></thead><tbody>${rowsB}</tbody></table><h2>5. 販売・トレーサビリティ記録</h2><table><thead><tr><th>日付</th><th>製品ロット</th><th>販売本数</th><th>販売先・注文番号</th></tr></thead><tbody>${rowsS}</tbody></table><p class="note">本記録は、採蜜ロット → 瓶詰め（製品）ロット → 販売記録を追跡できるよう作成しています。必要に応じて所管保健所の指導に合わせて運用・様式を見直してください。</p><script>setTimeout(()=>window.print(),300)<\/script></body></html>`);w.document.close();
}
...
function setActiveTab(p){document.querySelectorAll('nav button').forEach(b=>b.classList.toggle('active',b.dataset.page===p))}
function page(p){setActiveTab(p);({home,harvest,bottle,records,stock}[p]||home)()}
